...
Ideally, motion planners should be able to reason about physical and social interactions and adapt to changing conditions. NAMOSIM addresses this challenge by providing an open-source planner and simulation environment designed for single and multi-robot NAMO algorithms, which require not only navigation but also reasoning about which obstacles to move, where to move them, and how to coordinate with other agents.

While there are several prior works on NAMO [@wu_and_levihn; @levihn_2013; @levihn_2014; @scholz_2016; @zhang_2023], they lack multi-robot capability and do not provide reusable open-source tools. There is also a large body of work on multi-agent path finding (MAPF), but this does not consider movable obstacles. Another innovative related work [@baker2020emergenttoolusemultiagent] involved both multiple agents and movable obstacles, but was not designed for navigation nor for robotics applications. NAMOSIM fills this gap by providing a general-purpose software package for single and multi-robot NAMO. 

NAMOSIM supports reproducible research and real-world deployment via compatibility with the ROS [@ros2] ecosystem, facilitating integration with commonly-used packages such as Nav2 [@nav2] and Gazebo [@Koenig2004]. By providing a versatile tool for developing NAMO algorithms, NAMOSIM supports the robotics community to create more capable and adaptive robotic systems.

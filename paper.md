...
_NAMOSIM_ is a mobile robot navigation planner designed for the problem of Navigation Among Movable Obstacles (NAMO). It simulates robots navigating in 2D polygonal environments in which certain obstacles can be grabbed and relocated in order for the robots to reach their goals. NAMOSIM thus extends the classic navigation problem with a layer of interactivity which poses an interesting research questions while remaining well-defined and amenable to both classical and learning-based approaches. The simulator includes support for holonomic and differential-drive motion models, and integrates with ROS2 for visualization in RViz. NAMOSIM additionally supports multi-robot environments and provides a baseline communication-free coordination algorithm.

...
from graph_search import dist_between, heuristic_cost_estimate

from namosim.utils import utils


def two_way_multi_goal_a_star(
    grid,
    start_pose,
    intermediate_cells_and_poses,
    goal_pose,
    res,
    grid_pose,
    restrict_4_neighbors=False,
    authorize_goal_in_occupied_zone=False,
    ns="",
):
    ...
    paths_q_r_q_l = multi_goal_astar(
        grid,
        start_cell,
        intermediate_cells_and_poses.keys(),
        res,
        grid_pose,
        True,
        restrict_4_neighbors,
        ns=ns,
    )
    ...
    min_c_0_c_1_cost, min_c_0_disc_path, min_c_1_disc_path, best_obs_pose = (
        float("inf"),
        None,
        None,
        None,
    ...
    for cell, pose in intermediate_cells_and_poses.items():
        ...
        if c_0_c1_cost < min_c_0_c_1_cost:
            ...
            best_obs_pose = pose
    real_c0_path = utils.grid_path_to_real_path(
        min_c_0_disc_path, start_pose, best_obs_pose, res, grid_pose
    )
    real_c1_path = utils.grid_path_to_real_path(
        min_c_1_disc_path, best_obs_pose, goal_pose, res, grid_pose
    )
    return real_c0_path, real_c1_path


def multi_goal_a_star_real_path(
    grid,
    start_pose,
    goal_poses,
    res,
    ...
    reverse=True,
    restrict_4_neighbors=False,
    threshold_obstacle_value=1,
    ns="",
):
    start_cell = utils.real_to_grid(start_pose[0], start_pose[1], res, grid_pose)
    goal_cells = [
        utils.real_to_grid(goal_pose[0], goal_pose[1], res, grid_pose)
...
def multi_goal_astar(
    ...
    restrict_4_neighbors=False,
    threshold_obstacle_value=1,
    break_at_first_goal_found=False,
    ns="",
):
    ...
    while open_heap:
        ...
        for i, j in neighborhood:
            ...
            if (
                utils.is_in_matrix(neighbor, grid.shape[0], grid.shape[1])
                and neighbor not in close_set
                and (
                    grid[neighbor[0]][neighbor[1]] < threshold_obstacle_value
                    or neighbor in to_evaluate_set
                )
            ):
                ...
                if tentative_g_score < gscore[neighbor] or neighbor not in [
                    i.cell for i in open_heap
                ]:
                    ...
                    fscore[neighbor] = (
                        tentative_g_score
                        + _multi_heuristic_cost_estimate(neighbor, to_evaluate_set)
                    )
    ...
    for goal_cell in goals:
        try:
            paths.append(
                (
                    gscore[goal_cell],
                    _shortest_path(start_cell, goal_cell, came_from, reverse),
                )
            )
        except KeyError:

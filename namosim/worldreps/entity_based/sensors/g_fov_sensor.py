...
class GFOVSensor(CircularSectorSensor):
    def __init__(
        self,
        fov_max_radius: float,
        fov_min_radius: float,
        fov_opening_angle: float,
        parent_entity_pose: PoseModel,
    ):
    ...
    def _get_entities_in_fov_seethrough(self, world: "world.World"):
        ...
        for entity_uid, entity in world.entities.items():
            if entity_uid != self.parent_uid and entity.polygon.intersects(
                self.fov_polygon
            ):
                ...
                if isinstance(entity_visible_polygon, Polygon):
                    entities_in_fov[entity_uid] = Obstacle(
                        ...
                        pose=[
                            entity_visible_polygon.centroid.coords[0][0],
                ...
                try:
                    ...
                    if not target_entity.full_geometry_acquired:
                        if reference_entity.full_geometry_acquired:
                            target_world.set_entity_polygon(
                                target_entity.uid, reference_entity.polygon, True
                            )
                        else:
                            partial_polygon = cascaded_union(
                                [target_entity.polygon, reference_entity.polygon]
                            ).convex_hull
                            target_world.set_entity_polygon(
                                target_entity.uid, partial_polygon
                            )
                    # If it is already known, only translate/rotate the polygon appropriately
                    else:
                        if reference_entity.full_geometry_acquired:
                            translation = [
                                reference_entity.pose[0] - target_entity.pose[0],
                                reference_entity.pose[1] - target_entity.pose[1],
                            ]
                            rotation = (
                                reference_entity.pose[2] - target_entity.pose[2]
                            ) % 360.0
                            # Only apply translation if there is one
                            if not all(
                                np.isclose(translation, [0.0, 0.0], rtol=0.00001)
                            ):

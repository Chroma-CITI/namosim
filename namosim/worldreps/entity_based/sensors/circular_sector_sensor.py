...
class CircularSectorSensor:
    ...
    def _create_fov(
        ...
        fov_outer_arc = self._create_shapely_arc(
            parent_entity_pose, fov_max_radius, fov_opening_angle
        )

        fov_inner_arc = self._create_shapely_arc(
            parent_entity_pose, fov_min_radius, fov_opening_angle
        )
    ...
    def _create_shapely_arc(entity_pose, radius, opening_angle, numsegments=15):
        ...
        arc_after_trans = affinity.translate(
            arc_in_robot_ref, entity_pose[0], entity_pose[1]
        )
        arc_after_trans_rot = affinity.rotate(
            arc_after_trans, entity_pose[2], Point(entity_pose[0], entity_pose[1])
        )

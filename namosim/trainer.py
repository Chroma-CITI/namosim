...
class Example(t.NamedTuple):
    start_state: State
    goal_grid: npt.NDArray[np.float_]
    action: int
...
def render_trajectory(images: t.List[npt.NDArray[np.float_]]):
    # Create a window
...
def floating_img_to_uint8(img: npt.NDArray[np.float_]) -> npt.NDArray[np.uint8]:

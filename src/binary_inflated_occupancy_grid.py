...
class BinaryInflatedOccupancyGrid:
    def __init__(self):
        self._int_inflated_grid = None
        self._is_int_inflated_grid_valid = False
    ...
    def _update_int_grid(self, world):
        ...
        for prev_entity in self.prev_entities.values():
            if prev_entity.uid != world.robot_uid:
                prev_cells = prev_entity.get_discrete_inflated_cells_set(world.dd)
        ...
        for new_entity in self.new_entities.values():
            if new_entity.uid != world.robot_uid:
                new_cells = new_entity.get_discrete_inflated_cells_set(world.dd)

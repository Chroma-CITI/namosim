...
class SNAMOBehavior:
    ...
    def execute(self, q_init, q_goal, rp):
        ...
        self.robot = self.world.entities[self.robot_uid]
        rp.publish_goal(q_init, q_goal, self.world.entities[self.robot_uid].polygon)

        q_r = q_init
        e_l, m_l = [], []
        exec_success = True
        p_opt = [Plan([Path(a_star.a_star_real_path(self.world.get_inflated_grid(), q_r, q_goal, self.world.dd, rp))])]
        ...
        while not all(np.isclose(q_r, q_goal, rtol=0.00001)):
            ...
            rp.publish_robot_world(self.world)
            ...
            if not is_p_opt_valid or not exec_success:
                ...
                p_opt = [Plan([Path(a_star.a_star_real_path(self.world.get_inflated_grid(), q_r, q_goal, self.world.dd, rp))])]
                rp.publish_p_opt(p_opt[0])
                self.make_plan(q_r, q_goal, p_opt, e_l, m_l, rp)
    ...
    def make_plan(self, q_r, q_goal, p_opt, e_l, m_l, rp):
        # Update e_l
        ...
        while (min(self._get_cost_at_index(m_l, index_m_l), self._get_cost_at_index(e_l, index_e_l))
            ...
            if self._get_cost_at_index(m_l, index_m_l) < self._get_cost_at_index(e_l, index_e_l):
                ...
                if o_best_uid not in evaluated_obstacles_uids:
                    p_o_best = self.make_plan_for_obs(q_r, q_goal, o_best_uid, p_opt, rp)
    ...
    def make_plan_for_obs(self, q_r, q_goal, o_uid, p_opt, rp):
        ...
        for unit_translation, q_manip in obs.get_actions(self.world.dd, obs_is_push_only).items():
            c_1 = Path(a_star.a_star_real_path(self.world.get_inflated_grid(), q_r, q_manip, self.world.dd, rp), o_uid=o_uid)
            rp.publish_c_1(c_1)
            if not c_1.has_infinite_cost():
                ...
                if self.social_movability_evaluation_activated:
                    if self.robot.deduce_movability(obs.type) == "unknown":
                        ...
                        if q_look_index is not None:
                            ...
                            c_0, c_1 = self.compute_c_0_c_1(self.world, robot, obs, q_r, q_manip, rp)
                ...
                if c_0_is_valid and c_1_is_valid:
                    ...
                    total_translation, is_step_success, q_sim, c_est, target_obs_polygon = self._sim_one_step(
                        self.world, obs, [0.0, 0.0], unit_translation, q_manip, q_goal, c_1, init_robot_polygon, rp)
                    ...
                    while c_est <= p_opt[0].total_cost and is_step_success:
                        if (self._check_new_opening(self.world, obs, target_obs_polygon, q_goal)
                            ...
                            if self.use_social_layer:
                                social_cost = self.get_social_cost_for_entity(o_uid, self.world, world_copy, rp)
                                ...
                                c_2 = Path.line_path(q_manip, q_sim, weigth=self.manip_weight,
                                                     unit_translation=unit_translation, is_transfer=True, o_uid=o_uid)
                            rp.publish_c_2(c_2)
                            c_3 = Path(a_star.a_star_real_path(world_copy.get_inflated_grid(), q_sim, q_goal, world_copy.dd, rp),
                                       o_uid=o_uid)
                            rp.publish_c_3(c_3)
                            if not c_3.has_infinite_cost():
                                ...
                                if p.total_cost < p_best.total_cost:
                                    ...
                                    if p.total_cost < p_opt[0].total_cost:
                                        p_opt[0] = p
                                        rp.publish_robot_sim_costmap(world_copy)
                                        rp.publish_p_opt(p_opt[0])
                        # Increment one step
                        total_translation, is_step_success, q_sim, c_est, target_obs_polygon = self._sim_one_step(
    ...
    def compute_c_0_c_1(self, world, robot, obs, q_r, q_manip, rp):
        if self.social_movability_evaluation_activated:
            ...
            c_0_path, c_1_path = a_star.two_way_multi_goal_a_star(world.get_inflated_grid(), q_r, q_l, q_manip, world.dd, rp)

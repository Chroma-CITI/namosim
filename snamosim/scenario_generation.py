...
from snamosim.utils import utils, collision, conversion
...
def sample_poses_uniform(obstacles_polygons, robot_polygon, robot_pose, nb_poses=1, grid=None):
    ...
    while len(generated_poses) < nb_poses:
        ...
        if grid:
            rand_cell = utils.real_to_grid(rand_pose[0], rand_pose[1], grid.res, grid.grid_pose)
...
def generate_scenarios_alternatives(base_svg_filepath, nb_robots, nb_goals_per_robot, grid_res, nb_scenarios):
    # Load SVGs
    ...
    svg_base_elements_filepath = os.path.join(os.path.dirname(__file__), "../data/simulations/iros_2021/common_elements.svg")
    ...
    svg_base_robot_shape = svg_base_elements_data.getElementById("base_circular_like_robot_shape")
    svg_base_robot_direction = svg_base_elements_data.getElementById("base_circular_like_robot_direction")
    svg_base_goal_shape = svg_base_elements_data.getElementById("base_circular_like_robot_goal_shape")
    svg_base_goal_direction = svg_base_elements_data.getElementById("base_circular_like_robot_goal_direction")

    # Convert svg_data paths into polygons
    obstacles_polygons = {
        path.getAttribute("id"): conversion.svg_pathd_to_shapely_geometry(path.getAttribute('d'), scaling_value=0.01)
        for path in svg_data_init.getElementsByTagName('path')
    }
    base_robot_polygon = conversion.svg_pathd_to_shapely_geometry(svg_base_robot_shape.getAttribute('d'), scaling_value=0.01)
    base_robot_orientation_polygon = conversion.svg_pathd_to_shapely_geometry(svg_base_robot_direction.getAttribute('d'), scaling_value=0.01)
    base_robot_orientation_geom_coords = list(base_robot_orientation_polygon.coords)
    base_robot_orientation_vector = [
        base_robot_orientation_geom_coords[1][0] - base_robot_orientation_geom_coords[0][0],
        base_robot_orientation_geom_coords[1][1] - base_robot_orientation_geom_coords[0][1]
    ]
    base_robot_pose = (
    ...
    base_robot_orientation_pose = (
        base_robot_orientation_polygon.centroid.coords[0][0],
        base_robot_orientation_polygon.centroid.coords[0][1],
        base_robot_pose[2]
    )
    ...
    polygons_for_init_poses = {uid: p for uid, p in obstacles_polygons.items() if "direction" not in uid}
    all_obstacles_grid = BinaryInflatedOccupancyGrid(
        polygons_for_init_poses, grid_res, utils.get_circumscribed_radius(base_robot_polygon)
    )
    polygons_for_goals_poses = {
        uid: p for uid, p in obstacles_polygons.items()
        if not any(word in uid for word in ["movable", "box", "chair", "stool", "direction"])
    }
    only_static_obstacles_grid = BinaryInflatedOccupancyGrid(
        polygons_for_goals_poses, grid_res, utils.get_circumscribed_radius(base_robot_polygon)
    )
    ...
    for c_scenario in range(nb_scenarios):
        ...
        for i in range(nb_robots):
            goals_poses_for_robots.append(sample_poses_uniform(
                polygons_for_goals_poses, base_robot_polygon, base_robot_pose, nb_poses=nb_goals_per_robot, grid=only_static_obstacles_grid
            ))
        ...
        for i in range(nb_robots):
            ...
            conversion.add_shapely_geometry_to_svg(
                utils.set_polygon_pose(base_robot_polygon, base_robot_pose, initial_robot_poses[i]),
                robot_id + "_shape", svg_base_robot_shape.getAttribute('style'), svg_data, robot_group, scale=0.01
            )
            ...
            # Add robot goals
            robot_goals_layer = conversion.add_group(svg_data, robot_id + "_goals", goals_group)
            for goal_counter, goal_pose in enumerate(goals_poses_for_robots[i]):
                goal_id = robot_id + "_goal_" + str(goal_counter)
                goal_group = conversion.add_group(svg_data, goal_id, robot_goals_layer, is_layer=False)
                # Add goal shape
                conversion.add_shapely_geometry_to_svg(
                    utils.set_polygon_pose(base_robot_polygon, base_robot_pose, goal_pose),
                    goal_id + "_shape", svg_base_goal_shape.getAttribute('style'), svg_data, goal_group, scale=0.01
                )
        ...
        scenario_dirpath = os.path.join(base_svg_dirpath, str(nb_robots) + "_robots/", str(nb_goals_per_robot) + "_goals/", scenario_id + "/")
        if not os.path.exists(scenario_dirpath):
            os.makedirs(scenario_dirpath)
        scenario_world_svg_filename = "world_" + scenario_id + ".svg"
        with open(os.path.join(scenario_dirpath, scenario_world_svg_filename), "w+") as f:
            svg_data.writexml(f)
        ...
        world_json_data = {
            ...
            "files": {
                "geometry_file": "./" + scenario_world_svg_filename
            },
            "geometry_scale": 0.01,
                    ...
                    "goals": [
                        {
                            "geometry": {
                                "from": "file",
                                "id": "robot_" + str(c_robot) + "_goal_" + str(c_goals) + "_shape",
                                "orientation_id": "robot_" + str(c_robot) + "_goal_" + str(c_goals) + "_direction"
                            },
                            "name": "robot_" + str(c_robot) + "_goal_" + str(c_goals)
                        }
                        for c_robot in range(nb_robots) for c_goals in range(nb_goals_per_robot)
                    ]
                }
            }
        }
        scenario_world_json_filename = "world_" + scenario_id + ".json"
        with open(os.path.join(scenario_dirpath, scenario_world_json_filename), "w+") as f:
        ...
        simulation_json_data = {
            ...
            "files": {
                "world_file": "./" + scenario_world_json_filename
            },
            "provide_walls": True,
        }

...
from snamosim.behaviors.algorithms.graph_search import a_star_real_path
from snamosim.behaviors.plan.path import Path
from snamosim.behaviors.plan.plan import Plan
from snamosim.behaviors.algorithms.multi_goal_a_star import two_way_multi_goal_a_star
from snamosim.behaviors.algorithms.new_local_opening_check import (
    check_new_local_opening,
    is_move_passing_over_pose,
)
...
class WuLevihn2014Behavior(BaselineBehavior):
    ...
    def __init__(
        ...
        self._check_new_opening_activated = behavior_config["parameters"][
            "check_new_opening_activated"
        ]
        self._social_placement_choice_activated = behavior_config["parameters"][
            "social_placement_choice_activated"
        ]
        self._social_movability_evaluation_activated = behavior_config["parameters"][
            "social_movability_evaluation_activated"
        ]
        self._reset_knowledge_activated = behavior_config["parameters"][
            "reset_knowledge_activated"
        ]
    ...
    def think(self):
        if self._navigation_goals or self._q_goal is not None:
            ...
            if self._q_goal is None:
                ...
                self._world = (
                    copy.deepcopy(self._initial_world)
                    if self._reset_knowledge_activated
                    else self._world
                )
                q_r = self._robot.pose
                self._rp.publish_goal(
                    q_r, self._q_goal, self._robot, ns=self._robot_name
                )
            ...
            if self._last_action_result is not None:
                ...
                if last_action.is_transfer:
                    ...
                    is_last_action_success = isinstance(
                        self._last_action_result, ActionSuccess
                    )
                    if not is_last_action_success:
                        blocked_obstacle = self._world.entities[
                            last_action.obstacle_uid
                        ]
                        self._blocked_obstacles.add(blocked_obstacle.uid)
                        self._initial_world.add_entity(blocked_obstacle)

            if (
                not self._p_opt.is_valid(self._world, self._robot_uid)
                or self._last_action_result is None
            ):
                grid = self._world.get_binary_inflated_occupancy_grid(
                    (self._robot_uid,)
                ).get_grid()
    ...
    def make_plan(self, q_r, q_goal):
        ...
        for entity in self._world.entities.values():
            if isinstance(entity, Obstacle):
                ...
                if (
                    entity.uid not in self._blocked_obstacles
                    and entity_movability == "movable"
                    or entity_movability == "unknown"
                ):
                    c3_est = float("inf")
                    for q_manip in entity.get_actions(
                        self._world.dd.inflation_radius,
                        self._world.dd.res,
                        self._robot.deduce_push_only(entity.type),
                    ).values():
                        c3_est = min(
                            c3_est,
                            np.linalg.norm(
                                [q_goal[0] - q_manip[0], q_goal[1] - q_manip[1]]
                            ),
                        )
                        self.update_list(self._e_l, entity.uid, c3_est)
                elif (
                    entity_movability == "unmovable"
                    or entity.uid in self._blocked_obstacles
                ):
        ...
        while (
            min(
                self._get_cost_at_index(self._m_l, index_m_l),
                self._get_cost_at_index(self._e_l, index_e_l),
            )
            < self._p_opt.total_cost
        ):
            if self._get_cost_at_index(self._m_l, index_m_l) < self._get_cost_at_index(
                self._e_l, index_e_l
            ):
                ...
                if o_best_uid not in evaluated_obstacles_uids:
                    ...
                    if not p_o_best.has_infinite_cost():
                        self.update_list(
                            self._m_l,
                            o_best_uid,
                            p_o_best.path_components[1].phys_cost
                            + p_o_best.path_components[2].phys_cost,
                        )
    ...
    def make_plan_for_obs(self, q_r, q_goal, o_uid):
        ...
        self._rp.publish_q_manips_for_obs(
            obs.get_actions(
                self._world.dd.inflation_radius, self._world.dd.res, obs_is_push_only
            ).values(),
            ns=self._robot_name,
        )
        ...
        for unit_translation, q_manip in obs.get_actions(
            self._world.dd.inflation_radius, self._world.dd.res, obs_is_push_only
        ).items():
            grid = self._world.get_binary_inflated_occupancy_grid(
                (self._robot_uid,)
                ...
                if self._social_movability_evaluation_activated:
                    if self._robot.deduce_movability(obs.type) == "unknown":
                        ...
                        if q_look_index is not None:
                            ...
                            c_0, c_1 = self.compute_c_0_c_1(
                                self._world, robot, obs, q_r, q_manip
                            )
                        c_0_is_valid, c_1_is_valid = (
                            not c_0.has_infinite_cost(),
                            not c_1.has_infinite_cost(),
                        )

                if c_0_is_valid and c_1_is_valid:
                    init_robot_polygon = affinity.translate(
                        robot.polygon, q_manip[0] - q_r[0], q_manip[1] - q_r[1]
                    )
                    init_robot_polygon = affinity.rotate(
                        init_robot_polygon, q_manip[2] - q_r[2] % 360.0
                    )

                    self._rp.publish_sim(
                        init_robot_polygon, obs.polygon, "/init", ns=self._robot_name
                    )
                    ...
                    while c_est <= self._p_opt.total_cost and is_step_success:
                        if self._check_new_opening_activated:
                            ...
                            move_passes_over_goal = is_move_passing_over_pose(
                                moved_polygons, q_goal
                            )
                            is_it_worth_fully_evaluating = (
                                has_new_local_opening or move_passes_over_goal
                            )
                        else:
                        ...
                        if is_it_worth_fully_evaluating:
                            ...
                            if self._use_social_layer:
                                social_grid = world_copy.get_social_topological_occupation_cost_grid(
                                    (self._robot_uid, o_uid)
                                )
                                social_cost = world_copy.agg_grid_cost_for_entities(
                                    (o_uid,), social_grid
                                )
                                c_2 = Path.line_path(
                                    q_manip,
                                    q_sim,
                                    weigth=self._manip_weight,
                            ...
                            world_copy_grid = (
                                world_copy.get_binary_inflated_occupancy_grid(
                                    (self._robot_uid,)
                                ).get_grid()
                            )
                            c_3 = Path(
                            ...
                            if not c_3.has_infinite_cost():
                                ...
                                if p.total_cost < p_best.total_cost:
                                    ...
                                    if p.total_cost < self._p_opt.total_cost:
                                        self._p_opt = p
                                        self._rp.publish_robot_sim_costmap(
                                            world_copy, self._robot_uid
                                        )
    ...
    def _sim_one_step(
        ...
        target_obs_polygon = affinity.translate(
            obs.polygon, total_translation[0], total_translation[1]
        )
        self._rp.publish_sim(
            target_robot_polygon, target_obs_polygon, "/target", ns=self._robot_name
        )

        is_step_success = self._is_step_success(
            world,
            obs.uid,
            init_robot_polygon,
            target_robot_polygon,
        ...
        q_sim = (
            target_robot_polygon.centroid.coords[0][0],
            target_robot_polygon.centroid.coords[0][1],
            q_manip[2],
        )
        c_est = (
            c_1.phys_cost
            + np.linalg.norm(total_translation) * self._manip_weight
            + np.linalg.norm([q_goal[0] - q_sim[0], q_goal[1] - q_sim[1]])
        )
    ...
    def _is_step_success(
        ...
        robot_swept_area = cascaded_union(
            [init_robot_polygon, target_robot_polygon]
        ).convex_hull
        obs_swept_area = cascaded_union(
        ...
        for entity_uid, entity in world.entities.items():
            if entity_uid != self._robot_uid and entity_uid != o_uid:
                if entity.polygon.intersects(
                    robot_swept_area
                ) or entity.polygon.intersects(obs_swept_area):
                    return False
        return True
    ...
    def _get_last_look_q(self, robot, obs, path):
        ...
        while index != -1:
            ...
            displaced_fov_polygon = affinity.rotate(
                affinity.translate(robot.s_fov_polygon, trans[0], trans[1]), rot
            )
    ...
    def _split_at_pose(self, c_1_in, q_look_index, o_uid, c_0_in=None):
        ...
        c_0_out = Path(
            (c_0_in.path if c_0_in is not None else [])
            + (c_1_in.path[0 : q_look_index + 1] if not c_1_in_is_c_0 else c_1_in.path),
            is_observation=True,
            o_uid=o_uid,
        )
        c_1_out = Path(
            (
                c_1_in.path[q_look_index + 1 : len(c_1_in.path)]
                if not c_1_in_is_c_0
                else [c_1_in.path[len(c_1_in.path) - 1]]
            ),
            o_uid=o_uid,
        )
    ...
    def compute_c_0_c_1(self, world, robot, obs, q_r, q_manip):
        if self._social_movability_evaluation_activated:
            ...
            c_0_path, c_1_path = two_way_multi_goal_a_star(
                grid,
                q_r,
                q_l,
                q_manip,

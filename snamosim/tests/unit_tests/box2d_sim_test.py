...
class Box2DTest(unittest.TestCase):
    ...
    def test_basic_with_opening(self):
        ...
        polygons = {
            1: Polygon(
                [
                    (-1.3947547483983858, 1.5067685773149324),
                    (-1.4019618999238, 1.430010414020828),
                    (-1.4379945248779387, 1.3618531594840966),
                    (-1.4973669460672254, 1.31267315779076),
                    (-1.5710403014139362, 1.2899576101374424),
                    (-1.6477984929303193, 1.2971647616628565),
                    (-1.7159557474670506, 1.3331973583947168),
                    (-1.7651357491603874, 1.3925698078062823),
                    (-1.7878512968137048, 1.4662431631529929),
                    (-1.7806441452882906, 1.5430013264470972),
                    (-1.744611520334152, 1.6111585809838285),
                    (-1.685239099144865, 1.6603385826771653),
                    (-1.6115657437981543, 1.6830541303304827),
                    (-1.5348075522817712, 1.6758470070273472),
                    (-1.4666502977450397, 1.6398143820732085),
                    (-1.4174702960517032, 1.580441932661643),
                    (-1.3947547483983858, 1.5067685773149324),
                ]
            ),  # 'robot_01'
            2: Polygon(
        ...
        polygons[robot_uid] = affinity.translate(polygons[robot_uid], 0.3, 0.0)
        poses = {
            uid: (polygon.centroid.coords[0][0], polygon.centroid.coords[0][1], 0.0)
        ...
        init_welded_body_position, init_welded_body_angle = (
            tuple(b2_data.welded_body.position),
            b2_data.welded_body.angle,
        )
        ...
        for i in range(2):
            b2_data.welded_body.linearVelocity, b2_data.welded_body.angularVelocity = (
                (0.0, 0.0),
                math.radians(180.0),
            )

            b2_data.box2d_world.Step(
                timeStep=1, velocityIterations=1, positionIterations=1
            )

            collides = b2_data.contact_listener.is_collision_detected()

            if collides:
                b2_data.welded_body.position, b2_data.welded_body.angle = (
                    init_welded_body_position,
                    init_welded_body_angle,
                )
                (
                    b2_data.welded_body.linearVelocity,
                    b2_data.welded_body.angularVelocity,
                ) = (0.0, 0.0), 0.0
            ...
            new_robot_pose = (
                b2_data.welded_body.position[0],
                b2_data.welded_body.position[1],
                utils.angle_to_360_interval(math.degrees(b2_data.welded_body.angle)),
            )
            polygons[robot_uid] = utils.set_polygon_pose(
                polygons[robot_uid], poses[robot_uid], new_robot_pose
            )
            ...
            new_obstacle_pose = (
                new_obstacle_centroid_coords[0],
                new_obstacle_centroid_coords[1],
                utils.angle_to_360_interval(
                    init_obstacle_pose[2] + new_robot_pose[2] - init_robot_pose[2]
                ),
            )

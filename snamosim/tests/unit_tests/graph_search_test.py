...
def get_all_maps_names_and_filepaths(dirpath):
    return {
        f: {
            ...
            "scenarios_data": read_scenario_file(
                os.path.join(dp, "../", f + "-scen", f + ".scen")
            ),
        }
        for dp, dn, filenames in os.walk(dirpath)
        for f in filenames
        if (os.path.isfile(os.path.join(dp, f)) and os.path.splitext(f)[1] == ".map")
    }
...
def read_map_file(map_filepath):
    ...
    with open(map_filepath) as f:
        ...
        height = [int(s) for s in height_line.split() if s.isdigit()][
            0
        ]  # = grid lines count
        ...
        for y, line in enumerate(map_lines):
            for x, letter in enumerate(line):
                if letter == ".":
                    ...
                    raise NotImplementedError("Swamp letter not yet implemented.")
                elif letter == "W":
                    raise NotImplementedError("Water letter not yet implemented.")
                elif letter == "\n":
                    pass
                else:
                    raise ValueError(
                        "Only the letters [., G, @, O, T, S, W] are valid, found letter <{}>.".format(
                            letter
                        )
                    )
        return grid
...
class Scenario:
    def __init__(
        self, bucket, map_name, width, height, start_x, start_y, goal_x, goal_y, length
    ):
    ...
    def __str__(self):
        return "Start cell: {}, Goal cell: {}, Expected length: {}".format(
            self.start_cell, self.goal_cell, self.length
        )
...
class GraphSearchTest(unittest.TestCase):
    def setUp(self):
        ...
        self.benchmarks_dirname = os.path.join(
            self.dirname,
            "../../../data/thirdparties/gridsearch-dataset/www.movingai.com/benchmarks",
        )
    ...
    def for_scenario(
        ...
        if log_start:
            print(
                "Testing for map {}, scenario {} : {}".format(
        ...
        if path_found:
            ...
            measured_length = sum(
                [
                    utils.chebyshev_distance(cur_cell, raw_path[i + 1])
                    for i, cur_cell in enumerate(raw_path[:-1])
                ]
            )
            try:
        ...
        for map_counter, (map_name, my_map) in enumerate(self.dao_dataset_maps.items()):
            if multiprocess:
                ...
                p = multiprocessing.Process(
                    target=self.for_map, args=(map_name, my_map, map_counter)
                )
                processes.add(p)
                p.start()
            else:
        ...
        with open(target_scenario_filepath, "a") as target_scenario_file:
            for scenario in scenarios:
                (
                    ...
                    gscore,
                    _,
                ) = graph_search.grid_search_dijkstra(
                ...
                if path_found:
                    ...
                    new_scenario = Scenario(
                        scenarios[0].bucket,
                        scenarios[0].map_name,
                        scenarios[0].width,
                        scenarios[0].height,
                        scenarios[0].start_cell[0],
                        scenarios[0].start_cell[1],
                        scenario.goal_cell[0],
                        scenario.goal_cell[1],
                        measured_length,
                    )
    ...
    def test_dynamic_a_star(self, log_success=True):
        ...
        for scenario_counter, scenario in enumerate(scenarios):
            ...
            if search is None:
                search = graph_search.DynamicAStar(
                    start=scenario.start_cell, goal=scenario.goal_cell, graph=graph
                ).run()
            else:
            ...
            if path_dyn:
                measured_length = sum(
                    [
                        utils.chebyshev_distance(cur_cell, path_dyn[i + 1])
                        for i, cur_cell in enumerate(path_dyn[:-1])
                    ]
                )
                try:
                    self.assertAlmostEqual(scenario.length, measured_length, places=2)
